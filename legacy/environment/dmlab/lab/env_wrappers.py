...
logger = logging.getLogger("VizDoom Common Wrappers")
...
class PixelFormatChwWrapper(ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        if self.dict_obs_space:
            dtype = (env.observation_space.spaces["obs"].dtype
                     if env.observation_space.spaces["obs"].dtype is not None else np.float32)
        else:
