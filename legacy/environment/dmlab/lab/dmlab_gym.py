...
from .dmlab30 import DMLAB_INSTRUCTIONS, DMLAB_MAX_INSTRUCTION_LEN, DMLAB_VOCABULARY_SIZE
from .dmlab_level_cache import DmlabLevelCache
from .dmlab_utils import string_to_hash_bucket

logger = logging.getLogger("DMLab gym")
...
class DmlabGymEnv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        self.last_observation['obs'] = np.transpose(self.last_observation['obs'], (2, 0, 1))
    ...
    def step(self, action):
        ...
        if not terminated:
            ...
            obs_dict['obs'] = np.transpose(obs_dict['obs'], (2, 0, 1))
    ...
    def fetch(self, key, pk3_path):
        ...
        if not self.env_uses_level_cache:
            ...
            # logger.debug('Env %s uses level cache!', self.level_name)
        ...
        if os.path.isfile(path):
            ...
            logger.warning("Cache miss in environment %s key: %s!", self.level_name, key)
            return False

    def write(self, key, pk3_path):
        """Environment object itself acts as a proxy to the global level cache."""
        if self.use_level_cache:
            logger.debug("Add new level to cache! Level %s seed %r key %s", self.level_name,
                         self.last_reset_seed, key)
