...
PolicyID = int
logger = logging.getLogger("DMLab level cache")
    ...
    with open(filename, "r") as seed_file:
        ...
        for line in lines:
            try:
                ...
                logger.error("Could not read seed value from the file! File potentially corrupted")
                logger.exception("Exception when reading seeds file")
...
class DmlabLevelCache:
    ...
    def __init__(self, cache_dir, experiment_dir, all_levels_for_experiment, policy_idx, mp_ctx: BaseContext):
        ...
        logger.debug("Reading the DMLab level cache...")
        ...
        for lvl_seed_file in lvl_seed_files:
            ...
            logger.debug("Level %s has %d total seeds available", level, len(self.all_seeds[level]))

        logger.debug("Updating level cache for the current experiment...")
        ...
        for used_seeds_file in used_seeds_files:
            ...
            logger.debug("%d seeds already used in this experiment for level %s", len(self.used_seeds[level]),
                         level)
        ...
        for lvl in all_levels_for_experiment:
            ...
            logger.debug("Env %s has %d remaining unused seeds", lvl, len(self.available_seeds[lvl]))

        logger.debug("Done initializing global DMLab level cache!")
    ...
    def record_used_seed(self, level, seed):
        ...
        logger.debug("Updated number of used seeds for level %s (%d)", level,
            ...
            if num_used_seeds >= len(self.available_seeds.get(level, [])):
                ...
                while True:
                    ...
                    if new_seed in self.used_seeds[level]:
                        # logger.debug('Random seed %d already used in the past!', new_seed)
                        pass
                    else:
                        break
            else:
...
def make_dmlab_caches(
    ...
    for policy_id in range(num_policies):
        logger.info("Initializing level cache for policy %d...", policy_id)
