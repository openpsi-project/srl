...
from .action_space import Discretized
import logging

logger = logging.getLogger("VizDoom Environment")
...
def np_random(seed: Optional[int] = None) -> Tuple[np.random.Generator, Any]:
    ...
    if seed is not None and not (isinstance(seed, int) and 0 <= seed):
        if isinstance(seed, int) is False:
            raise logger.error(f"Seed must be a python integer, actual type: {type(seed)}")
        else:
            raise logger.error(f"Seed must be greater or equal to zero, actual value: {seed}")
...
def doom_lock_file(max_parallel):
    ...
    if not os.path.exists("/data/marl/checkpoints/mzy/.vizdoom"):
        try:
            os.makedirs("/data/marl/checkpoints/mzy/.vizdoom")
        except FileExistsError:
            pass
    lock_path = join(f"/data/marl/checkpoints/mzy/.vizdoom/", lock_filename)
    return lock_path
...
class VizdoomEnv(gym.Env):
    ...
    def __init__(
        ...
        if os.path.isabs(config_file):
            ...
            scenarios_dir = join(os.path.dirname(__file__), "../assets/scenarios")
    ...
    def seed(self, seed: Optional[int] = None):
        ...
        """
        self.rng, self.curr_seed = np_random(seed=seed)
    ...
    def _game_init(self, with_locking=True, max_parallel=1):
        lock_file = lock = None
        if with_locking:
        ...
        while True:
            ...
            try:
                ...
                logger.warning("VizDoom game.init() threw an exception %r. Terminate process...", exc)
    ...
    def reset(self, **kwargs) -> Tuple[np.ndarray, Dict]:
        ...
        if self.record_to and not self.is_multiplayer:
            ...
            logger.warning(f"Recording episode demo to {demo_path=}")

            if len(demo_path) > 101:
                logger.error(f"Demo path {len(demo_path)=}>101, will not record demo")
                logger.error(
        ...
        if img is None:
            logger.error("Game returned None screen buffer! This is not supposed to happen!")
    ...
    def _vizdoom_variables_bug_workaround(self, info, done):
        ...
        if done and "DAMAGECOUNT" in info:
            logger.info("DAMAGECOUNT value on done: %r", info.get("DAMAGECOUNT"))
    ...
    def close(self):
        try:
            ...
            logger.warning("Runtime error in VizDoom game close(): %r", exc)
    ...
    def play_human_mode(env, skip_frames=1, num_episodes=3, num_actions=None):
            ...
            while not doom.game.is_episode_finished() and not doom._terminate:
                ...
                for frame in range(skip_frames):
                    ...
                    if new_total_rew != total_rew:
                        logger.info("Reward: %.3f, total: %.3f", rew, new_total_rew)
        ...
        logger.debug("Press ESC to exit...")
    ...
    def replay(env, rec_path):
        ...
        while not doom.game.is_episode_finished():
            ...
            episode_reward += r
            logger.info("Episode reward: %.3f, time so far: %.1f s", episode_reward, time.time() - start)

        logger.info("Finishing replay")
        doom.close()
