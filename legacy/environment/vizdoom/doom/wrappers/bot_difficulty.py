...
logger = logging.getLogger("VizDoom bot difficulty")
...
class BotDifficultyWrapper(gym.Wrapper):
    ...
    def __init__(self, env, initial_difficulty=None):
        ...
        logger.info("Starting with bot difficulty %d", self._curr_difficulty)

        self._adaptive_curriculum = True
        if initial_difficulty == self._max_difficulty:
            logger.debug("Starting at max difficulty, disable adaptive skill curriculum")
