...
logger = logging.getLogger("VizDoom reward shaping wrapper")
EPS = 1e-5
NUM_WEAPONS = 8
...
class DoomRewardShapingWrapper(gym.Wrapper, RewardShapingInterface):
    ...
    def _selected_weapon_rewards(self, selected_weapon, selected_weapon_ammo, deltas):
        ...
        unholstered = len(self.selected_weapon) > 4 and all(sw == selected_weapon
                                                            for sw in self.selected_weapon)
        reward = 0.0
        ...
        if selected_weapon_ammo > 0 and unholstered:
            try:
                ...
                logger.error("%r", self.reward_shaping_scheme)
                logger.error("%r", selected_weapon)
    ...
    def _parse_info(self, info, done):
        ...
        if not done and not just_respawned:
            ...
            if abs(shaping_reward) > 2.5 and not self.print_once:
                logger.info("Large shaping reward %.3f for %r", shaping_reward, deltas)
        ...
        if done and "FRAGCOUNT" in self.reward_structure:
            ...
            logger.info("Sum rewards: %.3f, reward structure: %r", sum_rew, sorted_rew)
    ...
    def step(self, action):
        ...
        if self.verbose:
            logger.info("Original env reward before shaping: %.3f", self.orig_env_reward)
            player_id = 1
