...
logger = logging.getLogger("VizDoom multi-agent")
...
def is_udp_port_available(port):
    try:
        ...
        logger.warning(f"UDP port {port} cannot be used {str(exc)}")
        return False
    else:
        return True
...
def find_available_port(start_port, increment=1000):
    ...
    logger.debug("Port %r is available", port)
    return port
...
class VizdoomEnvMultiplayer(VizdoomEnv):
    ...
    def _ensure_initialized(self):
        ...
        if self._is_server():
            logger.info("Using port %d on host...", port)
            ...
            if self.record_to is not None:
                ...
                logger.debug("Recording multiplayer demo to %s", demo_path)
        ...
        logger.info("Initialized w:%d v:%d player:%d", self.worker_index, self.vector_index, self.player_id)
    ...
    def reset(self, **kwargs):
        ...
        if self._is_server() and self.num_bots > 0:
            ...
            #         else:
            #             bot_name = ""
            #         # logger.info('Adding bot %d %s', i, bot_name)
    ...
    def step(self, actions):
        ...
        if self.record_to is not None:
            ...
            if self.game.get_episode_time() + 1 == self.game.get_episode_timeout():
                logger.debug("Calling stop recording command!")
