...
from .types import accepts, returns, maybe
...
def get_camera_xyaxes(camera_pos, target_pos):
    ...
    assert camera_pos.shape == (3,), "Bad camera position {}".format(camera_pos)
    assert target_pos.shape == (3,), "Bad target position {}".format(target_pos)
...
def recursive_rename(xml_dict, prefix):
    attrs = [
        "@name", "@joint", "@jointinparent", "@class", "@source", "@target", "@childclass", "@body1",
        "@body2", "@mesh", "@joint1", "@joint2", "@geom", "@geom1", "@geom2", "@site", "@material",
        "@texture", "@tendon", "@sidesite", "@actuator"
    ]
    names = [
        "geom", "joint", "jointinparent", "body", "motor", "freejoint", "general", "position", "default",
        "weld", "exclude", "mesh", "site", "pair", "jointpos", "touch", "texture", "material", "fixed",
        "spatial", "motor", "actuatorfrc"
    ]
    ...
    for key in list(xml_dict.keys()):
        ...
        if key in names:
            assert isinstance(value_dict, list), "Invalid type for value {}".format(value_dict)
