...
from ..util.types import accepts, returns
from ..parser import update_mujoco_dict
from ..transforms import closure_transform
from ..util.obj_util import get_name_index, get_axis_index
from ..util.placement import place_boxes
...
class Obj(object):
    ...
    @accepts(object, str, (tuple, list, np.ndarray), (tuple, list, np.ndarray), str,
             (tuple, list, np.ndarray))
    def mark(self,
             mark_name,
             relative_xyz=(0.5, 0.5, 0.5),
             absolute_xyz=None,
             rgba=None,
             geom_type="sphere",
    ...
    def generate(self, world_params, random_state, placement_size):
        raise NotImplementedError("This needs to be implemented in child classes.")
    ...
    def compile(self, random_state, world_params):
        ...
        for placement_name, children in list(self.children.items()):
            ...
            matches = [pn for pn in candidates if pn.startswith(placement_name)]
            assert len(matches) > 0, ("No match found in {} for {}".format(placement_name, self.name))
            ...
            for _ in range(max_tries):
                ...
                for child, _ in self.children[placement_name]:
                    ...
                    if not child.placeable:
                        continue
                    assert child.size is not None, "missing size {}".format(child)
                    assert child.placements is not None, "missing placements {}".format(child)
                    child.relative_position = None
                success = self.place(placement_name, self.children[placement_name], random_state,
                                     world_params.placement_margin)
                if success:
                    for child, _ in self.children[placement_name]:
                        if not child.placeable:
                            continue
                        assert child.relative_position is not None, "{}".format(child)
                    break  # next
            if not success:
    ...
    # Not recursive.
    def place(self, placement_name, children, random_state, placement_margin):
        ...
        boxes = [{"size": c[0].size, "placement_xy": c[1]} for c in placeable_children]
        width, height, _ = placement['size']
        locations = self.place_boxes(random_state, boxes, width, height, placement_margin=placement_margin)
    ...
    def to_xml_dict(self):
                ...
                if jnt["@type"] == "slide":
                    ...
                    if self._keep_slide_joint[axis_idx]:
                        ...
                        body["@pos"][axis_idx] = float(
                            body["@pos"][axis_idx]) + self.absolute_position[axis_idx]
                elif jnt["@type"] == "hinge":
                    axis_idx = get_axis_index(jnt["@axis"])
        ...
        if len(self.markers) > 0:
            bodies = [
                ...
                if "annotation" not in body["@name"] and ("@mocap" not in body or not body["@mocap"])
            ]
