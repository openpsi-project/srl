...
from .obj import Obj
from ..util.obj_util import establish_size, get_body_xml_node
from ..util.types import store_args


class Geom(Obj):

    @store_args
    def __init__(self, geom_type, min_size=None, max_size=None, name=None, rgba=None):
    ...
    def generate(self, random_state, world_params, placement_size):
        ...
        if self.geom_type == "box":
            ...
            top = OrderedDict(origin=(0, 0, self.size[2]), size=(self.size[0], self.size[1], top_height))
            self.placements['top'] = top
        elif self.geom_type == "sphere":
            min_size = np.max(min_size)
            max_size = np.min(max_size)
            assert (min_size <= max_size)
            self.size = np.ones(3) * (min_size + (max_size - min_size) * random_state.uniform(size=1))
