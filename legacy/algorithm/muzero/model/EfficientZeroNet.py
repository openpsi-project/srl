...
def mlp(
    ...
    if output_size != -1 and init_zero:
...
class PredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.fc_policy = mlp(self.block_output_size_policy,
                             ...
                             init_zero=False,
                             momentum=momentum)
        self.act_layer = ACTLayer(action_space, fc_policy_layers[-1])
    ...
    def forward(self, x):
        ...
        action_sampler = self.act_layer(self.fc_policy(policy))
...
class EfficientZeroNet(BaseNet):
    ...
    def __init__(
        ...
        """
        super(EfficientZeroNet, self).__init__(act_dim,
                                               action_space,
                                               inverse_value_transform,
                                               inverse_reward_transform,
                                               lstm_hidden_size,
        ...
        assert (self.action_space.__class__.__name__ == "Discrete"), (
            "EfficientZeroNet only works for discrete action space now.")
        ...
        self.projection = nn.Sequential(nn.Linear(self.porjection_in_dim, self.proj_hid),
    ...
    def prediction(self, encoded_state):
        action_sampler, value = self.prediction_network(encoded_state)
        return action_sampler, value
    ...
    def dynamics(self, encoded_state, reward_hidden, action):
        ...
        action_one_hot = (action[:, :, None, None] * action_one_hot / self.action_space.n)
