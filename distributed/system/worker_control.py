...
WORKER_JOB_STATUS_LINGER_SECONDS = 3600 * 24 * 365
...
class ZmqServer(worker_base.WorkerServer):
    ...
    def set_status(self, status: worker_base.WorkerServerStatus):
        ...
        distributed.base.name_resolve.add(
            ...
            value=status.value,
            keepalive_ttl=60,  # Job Status lives for a day.
            replace=True,
    ...
    def __init__(
        ...
        try:
            controller_status = distributed.base.name_resolve.wait(names.worker_status(
        ...
        if experiment_name is not None and trial_name is not None:
            ...
            distributed.base.name_resolve.add(key, address, keepalive_ttl=10, delete_on_exit=True)
...
class ZmqWorkerControl(worker_base.WorkerControlPanel):
    ...
    def connect(self,
        ...
        for name in worker_names:
            ...
            try:
                ...
                server_address = distributed.base.name_resolve.wait(names.worker(
    ...
    def auto_connect(self):
        ...
        worker_names = [r[len(name_root):] for r in distributed.base.name_resolve.find_subtree(name_root)]
    ...
    def get_worker_status(self, worker_name) -> distributed.system.worker_base.WorkerServerStatus:
        try:
            status_str = distributed.base.name_resolve.wait(
                names.worker_status(experiment_name=self.__experiment_name,
                                    trial_name=self.__trial_name,
                                    worker_name=worker_name),
                timeout=60,
            )
            status = distributed.system.worker_base.WorkerServerStatus(status_str)
        except distributed.base.name_resolve.NameEntryNotFoundError:
            status = distributed.system.worker_base.WorkerServerStatus.LOST
        return status
